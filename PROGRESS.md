...
## 📅 2026-09-03 (Thu) — Round 4 fixes: Figma-intake fallback, PENDING propagation, persona-driven gaps closed

### What changed
Fixed all 25 findings from the 4th test round (persona/generalization testing across a content designer, a junior PM, and a product designer persona — see 2026-09-02's entry below). Ran 5 parallel agents, one per skill, plus 2 direct edits to the shared conventions doc:

- **`references/output-status-and-labeling-conventions.md`** (direct edit): added PENDING propagation + retirement rules (previously only BLOCKED had one); a tool-limitation case now correctly resolves to PENDING not BLOCKED; extended the TEST ARTIFACT label from 3 researcher-led skills to the whole repo.
- **`diy-triage`**: documented Figma-link fallback (retry-after-share-confirm, then fall back to description/screenshot, mark PENDING downstream) — **the highest-leverage fix in this round**, since it hit 2 of 3 persona chains independently; a "Requester confidence note" field for low-engagement requesters; a "validated sub-questions" scope note so downstream skills don't need to re-read the raw transcript; a middle option added to Q3.
- **`diy-research-plan`**: deferral-vs-genuine-agreement guidance at the confirmation checkpoints; the stimulus gate now distinguishes BLOCKED (requester hasn't produced it) from PENDING (tool can't fetch it); plain-language fixes in Section 4; a deliberately-scoped-count carve-out; a no-researcher-assigned fallback.
- **`screener`** (+ question bank): a new own-account consent gate; persistent-state and performed-vs-received-outcome question patterns; a session-specific hard device gate distinct from the habits inventory.
- **`unmod-script`**: a no-link/live-feature branch for Stimuli + Launch; the same deliberately-scoped-count carve-out; a monadic-then-comparison hybrid option; a time-budget sanity check; a literal-on-screen-copy self-critique item; a cross-artifact number-conflict check.
- **`diy-packet`**: a conflict-surfacing exception to Rule 1 (don't silently pick a number when upstream artifacts disagree); a no-link fit-gate branch; a same-session cold-pickup skip; Rule 7 extended to any upstream skill's flagged note, not just the plan's; the same task-count carve-out; an already-resolved-live check; a Session length field.

Confidentiality grep clean (no persona names leaked). All committing + pushing next.

### Open / next
- No known open majors across all 4 rounds now.
- `mod-guide`'s accumulated fixes (rounds 3+4) still ride uncommitted on top of its unrelated June WIP.
- No 5th round planned yet — 4 rounds (bug-hunt → retest → nit-batch → persona-generalization) covers both correctness and generalization; further testing would be diminishing returns unless a new persona/scenario shape comes up.
