...
- [ ] **Upstream flagged items reflect their current status, not their original one.** If the requester already resolved an item earlier in this same conversation — answered an open question, cleared a pending asset, addressed a fit-gate concern — the packet notes it as resolved instead of re-flagging it as still open.
- [ ] **No question added beyond an existing upstream script.** The comprehension-check suggestion for comparison tests only applies when this skill is drafting questions from scratch (no `/unmod-script` output to assemble) — when an `/unmod-script` output exists, every question in the packet should already be in it.
- [ ] **Section 2 carries forward the plan's "why it matters" field**, not just the learning goal — a requester filling this out shouldn't lose the rationale the upstream `/diy-research-plan` already captured.
...
- [ ] **Escalation triggers are concrete examples, not a vague "if you get stuck."** A non-researcher doesn't reliably know what "stuck" means in a research context. Name the actual signals (confused by the test itself, wants to ask a follow-up, results feel bigger than the original question). Where `/diy-research-plan`'s Section 6 already named study-specific triggers, those should be merged in, not dropped in favor of the generic four.
